...
import static acmecollege.entity.SecurityRole.FIND_ROLE_BY_NAME_QUERY_NAME;
...
import static acmecollege.entity.SecurityUser.FIND_USER_BY_NAME_QUERY_NAME;
...
public class ACMECollegeService implements Serializable {
    ...
    public void buildUserForNewStudent(Student newStudent) {
        ...
        SecurityRole userRole = em.createNamedQuery("SecurityRole.findByRoleName",SecurityRole.class).setParameter("roleName", USER_ROLE).getSingleResult(); /* TODO ACMECS01 - Use NamedQuery on SecurityRole to find USER_ROLE */ ;
    ...
    public void deleteStudentById(int id) {
        ...
        if (student != null) {
            em.refresh(student);
            TypedQuery<SecurityUser> findUser = em.createNamedQuery(FIND_USER_BY_NAME_QUERY_NAME,SecurityUser.class);
    ...
    public <T> List<T> getAllWithId(Class<T> entity, String namedQuery, int id) {
    ...
    public Course deleteCourseById(int id) {
    	Course c = getById(Course.class, Course.FIND_BY_ID_QUERY, id);
    			if(c!= null)em.remove(c);
    	return c;
    }

    @Transactional
    public Professor persistProfessor(Professor p) {
    	em.persist(p);
    	return p;	
    }
    ...
    public Professor deleteProfessorByID(int id){
    	Professor p = getById(Professor.class, Professor.FIND_BY_ID_QUERY, id);
    ...
    public void deleteMembershipCardById(int id) {
        MembershipCard card = getById(MembershipCard.class, MembershipCard.ID_CARD_QUERY_NAME, id);
        if (card != null) {
            em.remove(card);
        }
    }
    ...
    //StudentClub
    //Create already exists above
    
    @Transactional
    public void deleteStudentClubById(int id) {
        StudentClub club = getById(StudentClub.class, StudentClub.SPECIFIC_STUDENT_CLUB_QUERY_NAME, id);
        if (club != null) {
            Set<ClubMembership> memberships = club.getClubMemberships();
            memberships.forEach(m -> {
                if (m.getCard() != null) {
                    MembershipCard card = m.getCard();
